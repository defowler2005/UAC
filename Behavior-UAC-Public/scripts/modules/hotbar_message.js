        ...
        if (player.hasTag(configuration.staff_tag)) {
            ...
            if (hmm_toggle >= 1) {
                if (player.getGameMode() === GameMode.creative) {
                    if (opam_toggle === 0) {
                        if (hometp === 3) {
                            return hotbar(player, `§¶§aCREATIVE ENABLED §7| §d'${configuration.prefix}help' §7| §7[§2v${configuration.uac_version[0]}§7.§2${configuration.uac_version[1]}§7.§2${configuration.uac_version[2]}§7]\n§bEntity Count §7: ${entitycount}\n§bPlayer Count §7: ${playercount}\n§bCurrent WorldSpawn§7: §cX §7= ${SpawnX} §2Y §7= ${SpawnY} §3Z §7= ${SpawnZ}`);
                        }
                        if (hmm_toggle === 2) {
                            if (hometp === 3) {
                                return hotbar(player, '§¶§aCREATIVE ENABLED §7| §d` /Function UAC/help/all-commands ` §7| §7[§2v${configuration.uac_version[0]}§7.§2${configuration.uac_version[1]}§7.§2${configuration.uac_version[2]}§7]§b')
                            }
                        }
                    }
                } else if (player.getGameMode() === GameMode.spectator) return hotbar(player, `§¶§d    SPECTATOR MODE \n §¶§bPlayers§7: §c${playercount} §7|| §bEntities§7: §c${entitycount}`);
            } else if (hmm_toggle == 0) {
                // self stats display
                if (hometp === 1337) hotbar(player, `    §¶§bUAC §7[§2v${configuration.uac_version[0]}§7.§2${configuration.uac_version[1]}§7.§2${configuration.uac_version[2]}§7] Public\n§¶§bkills§7: §c${kills}\n§bdeaths§7: §c${deaths}\n§bkillstreak§7: §c${killstreak} \n§bMoney §c$${money}`);
                // server stats display
                if (hometp === 420) hotbar(player, `     §¶§bUAC §7[§2v${configuration.uac_version[0]}§7.§2${configuration.uac_version[1]}§7.§2${configuration.uac_version[2]}§7] Public\n§¶§bPlayers §7: §c${playercount} §7|| §bEntities §7 : §c${entitycount}`);
            }
        };

        if (player.getGameMode() === GameMode.survival) {
            //with scors
            if (hmm_toggle == 1) return hotbar(player, `§¶§bUAC §7[§2v${configuration.uac_version[0]}§7.§2${configuration.uac_version[1]}§7.§2${configuration.uac_version[2]}§7] Public\n§¶§bkills§7: §c${kills}\n§bdeaths§7: §c${deaths}\n§bkillstreak§7: §c${killstreak}\n§bMoney §7: §c$${money}`);
            //without score
            if (hmm_toggle == 2) return hotbar(player, `§¶§bUAC §7[§2v${configuration.uac_version[0]}§7.§2${configuration.uac_version[1]}§7.§2${configuration.uac_version[2]}.§27§7] Public`);
            //resource mode
...
execute @a[scores={hmmtoggle=2,hometp=3}] ~~~ titleraw @s actionbar {"rawtext":[{"text":"§¶§aCREATIVE ENABLED §7| §d` /Function UAC/help/all-commands ` §7| §7[§2v${configuration.uac_version[0]}§7.§2${configuration.uac_version[1]}§7.§2${configuration.uac_version[2]}§7]"}]}

#creative resource mode
execute @a[scores={hmmtoggle=3,OPAM=0,hometp=3}] ~~~ titleraw @s actionbar {"rawtext":[{"text":"§¶§aCREATIVE ENABLED §7| §d` /Function UAC/help/all-commands ` §7| §7[§2v${configuration.uac_version[0]}§7.§2${configuration.uac_version[1]}§7.§2${configuration.uac_version[2]}§7]§b\n §bEntity Count §7: "},{"score":{"name":"entitydummy","objective":"entitycount"}},{"text":" §bPlayer Count §7: "},{"score":{"name":"playerdummy","objective":"playercount"}},{"text":" §bCurrent WorldSpawn§7: X = "},{"score":{"name":"@s","objective":"Worldx"}},{"text":" Z = "},{"score":{"name":"@s","objective":"Worldz"}}]}
