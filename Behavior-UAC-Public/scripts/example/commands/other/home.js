...
Server.command.register(registerInformation, (chatmsg, args) => {
    try {
        system.run(() => {
            ...
            if (Database.get('icmtoggle') !== 1) {
            ...
            } else if (scoreTest(sender, 'tp_cooldown') >= 1) {
                var cooldownSeconds = scoreTest(sender, 'tp_cooldown');
                var minutes = Math.floor(cooldownSeconds / 60);
                var seconds = cooldownSeconds % 60;
                sender.tellraw(`§¶§cUAC ► §6Home §cunavailable §bwhile warp commands are in cooldown. Please wait ${minutes} minute${minutes !== 1 ? 's' : ''} and ${seconds} second${seconds !== 1 ? 's' : ''}.`);
                return;
            } else if (Database.get('icmtoggle') === 1) {
                ...
                else return sender.tellraw("§¶§cUAC ► §6USAGE §7: §bUAC.home <list | set | remove | warp> [home name]");
            }
        });
    } catch (error) {
        console.warn(`An error occured while processing the action ${registerInformation.name}${args[0]}: ${error}\n${error.stack}`);
    }
});