...
if (!Database.get('toggle')) Database.set('toggle', 0)
if (!Database.get('max_dist')) Database.set('max_dist', 6000)

// note that this is multiplied by 1000 to support decimals.
let maxdist = Database.get('max_dist')

let toggle = Database.get('toggle')
...
Server.command.register(registerInformation, (chatmsg, args) => {
    ...
    switch (args[0]) {
        ...
        case 'max-reach': {
            ...
            maxdist = newValue;
            Database.set('max_dist', newValue)
...
World.afterEvents.entityHurt.subscribe(({ entity: plr, hurtEntity: target }) => {
